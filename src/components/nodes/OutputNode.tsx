...
export function OutputNode({ data, selected }: NodeProps<OutputNodeType>) {
  return (
    <div className={`relative ${selected ? 'ring-2 ring-offset-1 ring-emerald-500 rounded-xl' : ''}`}>
      ...
      <Card className="min-w-[160px] shadow-md p-0 overflow-hidden">
        ...
        <CardContent className="px-3 py-2 min-h-[32px] flex flex-wrap gap-1">
          {Object.entries(data.parameters).map(([k, v]) => (
            <span key={k} className="text-xs bg-emerald-50 text-emerald-700 rounded-full px-2 py-0.5 font-medium">
              {k}: {v}
            </span>
          ))}
        </CardContent>
