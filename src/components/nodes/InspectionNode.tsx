...
export function InspectionNode({ data, selected }: NodeProps<InspectionNodeType>) {
  return (
    <div className={`relative ${selected ? 'ring-2 ring-offset-1 ring-amber-500 dark:ring-offset-gray-900 rounded-xl' : ''}`}>
      ...
      <Card className="min-w-[160px] shadow-md p-0 overflow-hidden dark:bg-gray-800 dark:border-gray-700">
        ...
        <CardContent className="px-3 py-2 min-h-[32px] flex flex-wrap gap-1">
          {Object.entries(data.parameters).map(([k, v]) => (
            <span key={k} className="text-xs bg-amber-50 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 rounded-full px-2 py-0.5 font-medium">
              {k}: {v}
            </span>
          ))}
        </CardContent>
